...
func Put(tx RwTx, bucket string, key []byte, value []byte) error {
	return tx.RwCursor(bucket).Put(key, value)
}

func Append(tx RwTx, bucket string, key []byte, value []byte) error {
	return tx.RwCursor(bucket).Append(key, value)
}

func AppendDup(tx RwTx, bucket string, key []byte, value []byte) error {
	return tx.RwCursorDupSort(bucket).AppendDup(key, value)
}

func Delete(tx RwTx, bucket string, k, v []byte) error {
